define("MoneyMovementModule/userfrmMMTransferToAccountController", {
    sec: -1,
    row: -1,
    segmentHeight: 0,
    payeesList: 0,
    ...
    searchData: [],
    viewConstants: {
        "Infinity": "temenos.png",
        "Icici": "temenos.png",
        "Boa": "boa.png",
        "Citi": "citi.png",
        "National": "temenos.png"
    },
    init: function() {
        var scope = this;
    ...
    navigateCustomBack: function() {
        if (kony.os.deviceInfo().name === "iPhone") this.view.customSearchbox.tbxSearch.setFocus(false);
        ...
        if (this.sec !== "" && this.row !== "" && this.sec !== -1 && this.row !== -1) {
            if (!(this.sec === 0 && this.row === 0)) {
                if ((kony.os.deviceInfo().name !== "iPhone")) {
                    ...
                    if (parseInt(this.segmentHeight) > this.view.segTransactions.frame.height || (this.view.segTransactions.frame.height - (this.segmentHeight)) < this.rowTemplateHeight) this.scrollManually();
                } else {
        ...
        if (moneyMovementModule.getEntitlementValue("isInternationalAccountsTransfer") == "true" || moneyMovementModule.getEntitlementValue("isOtherBankAccountsTransfer") == "true" || moneyMovementModule.getEntitlementValue("isOtherKonyAccountsTransfer") == "true") {
            ...
            if (configurationManager.isCombinedUser === "true") {
                for (var k = 0; k < proccessToData.length; k++) {
                    proccessToData[k]["accountTypeFlx"] = {
                        "isVisible": true
                    };
                    proccessToData[k].bankName = {
                            "text": proccessToData[k].bankName,
                    ...
                    if (!kony.sdk.isNullOrUndefined(proccessToData[k]["bankImage"])) {
                        proccessToData[k]["flxBankIconType"] = {
                            "isVisible": true
                        };
                        proccessToData[k]["imgBankIconType"] = {
                            "src": proccessToData[k]["bankImage"]
                        };
                    } else {
                        proccessToData[k]["flxBankIconType"] = {
                            "isVisible": false
            ...
            if (this.payeesList && this.payeesList.length > 0) {
                ...
                if (configurationManager.isCombinedUser === "true") {
                    for (var x = 0; x < proccessPayeeData.length; x++) {
                        proccessPayeeData[x]["accountTypeFlx"] = {
                            "isVisible": false
                        };
                        proccessPayeeData[x]["accountTypeIcon"] = {
                            "src": "personalaccount.png"
                        };
                        proccessPayeeData[x].bankName = {
                            "text": proccessPayeeData[x].bankName,
                            "skin": "sknlbl727272Brf6f6f6Reg98"
                        }
                    }
                } else {
    ...
    getListOfToAccountsForMultipleIDs: function() {
        ...
        var externalPersonalAccounts = [{
                "lblHeader": ""
            },
            []
        ];
        var externalBusinessAccounts = [{
                "lblHeader": ""
            },
            []
        ];
    ...
    categorizeDataforMultipleIDs: function(mappingData) {
        ...
        for (let key in mappingData) {
            if (mappingData[key].isPersonal !== true) {
                ...
                if (mappingData[key].accounts.length > 0) sectionHeaderforMultipleIDs["lblHeader"] = mappingData[key].accounts[0].membershipName + "(" + mappingData[key].accounts.length + ")";
    ...
    scrollManually: function() {
        ...
        var parallaxSpeed = 1;
        if (Math.abs(this.view.segTransactions.frame.height - (this.segmentHeight)) < this.rowTemplateHeight && this.payeesList !== 0) var yOffset = this.view.segTransactions.contentOffsetMeasured.y + this.sectionTemplateHeight;
        else var yOffset = this.view.segTransactions.contentOffsetMeasured.y + (2 * this.rowTemplateHeight);
        if (this.view.flxHeaderSearchbox.height === "40dp") this.view.flxDescription.top = 40 - (yOffset * parallaxSpeed) + "dp";
        else this.view.flxDescription.top = 55 - (yOffset * parallaxSpeed) + "dp";
        this.view.flxSearch.top = 0 - (yOffset * parallaxSpeed) + "dp";
        this.view.flxGradient.top = 0 - (yOffset * parallaxSpeed) + "dp";
    ...
    getBankIcon: function(bankName) {
        try {
            ...
            if (!kony.sdk.isNullOrUndefined(bankName)) {
                if (bankName === "Infinity") img = this.viewConstants.Infinity
                else if (bankName === "ICICI bank") img = this.viewConstants.Icici
                else if (bankName === "Bank of America" || bankName === "america") img = this.viewConstants.Boa
                else if (bankName === "CitiBank") img = this.viewConstants.Citi
                else if (bankName === "National Bank") img = this.viewConstants.National
                else img = this.viewConstants.Infinity
            }
            return img;
        } catch (er) {
            kony.print(er);
...
define("MoneyMovementModule/frmMMTransferToAccountControllerActions", {
    /*
    ...
    */
    ...
    AS_Form_j6bcaa29ef104cce8d81ac25694cb9bd: function AS_Form_j6bcaa29ef104cce8d81ac25694cb9bd(eventobject) {
        var self = this;
        this.postShow();
    },
    ...
    AS_BarButtonItem_ee01dc0a42d641bd94fce28238c3ccbe: function AS_BarButtonItem_ee01dc0a42d641bd94fce28238c3ccbe(eventobject) {
        var self = this;
        this.navigateCustomBack();
    },
    AS_BarButtonItem_g226f0d3525d4027adc3e457cc2fcb9e: function AS_BarButtonItem_g226f0d3525d4027adc3e457cc2fcb9e(eventobject) {
        var self = this;
        this.cancelOnClick();
    }
});
define("MoneyMovementModule/frmMMTransferToAccountController", ["MoneyMovementModule/userfrmMMTransferToAccountController", "MoneyMovementModule/frmMMTransferToAccountControllerActions"], function() {
    var controller = require("MoneyMovementModule/userfrmMMTransferToAccountController");
    var controllerActions = ["MoneyMovementModule/frmMMTransferToAccountControllerActions"];
