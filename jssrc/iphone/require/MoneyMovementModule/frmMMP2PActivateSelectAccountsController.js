define("MoneyMovementModule/userfrmMMP2PActivateSelectAccountsController", ['CampaignUtility'], function(CampaignUtility) {
    return {
        segmentData: null,
        sec: -1,
        row: -1,
                    ...
                    if (configurationManager.isCombinedUser === "true") {
                        try {
                            ...
                            for (var k = 0; k < combinedUserData.length; k++) {
                                if (combinedUserData[k]["isBusiness"] == 1) accProcessedData[1][1][b++] = combinedUserData[k];
                                else if (combinedUserData[k]["isBusiness"] == 0) accProcessedData[0][1][p++] = combinedUserData[k];
                                else er = 1;
                            }
                            if (kony.sdk.isNullOrUndefined(kony.i18n.getLocalizedString("kony.mb.fastTransfers.BusinessAccounts"))) accProcessedData[1][0]["lblHeader"] = "Business Accounts" + " (" + b + ")";
                            else accProcessedData[1][0]["lblHeader"] = kony.i18n.getLocalizedString("kony.mb.fastTransfers.BusinessAccounts") + " (" + b + ")";
                            if (kony.sdk.isNullOrUndefined(kony.i18n.getLocalizedString("kony.mb.fastTransfers.personalAccounts"))) accProcessedData[0][0]["lblHeader"] = "Personal Accounts" + " (" + p + ")";
                            else accProcessedData[0][0]["lblHeader"] = kony.i18n.getLocalizedString("kony.mb.fastTransfers.personalAccounts") + " (" + p + ")";
                            if (er === 0 && p !== 0 && b !== 0) {
                ...
                for (j = 1; j < data[i][1].length; j++) {
                    if (kony.sdk.isNullOrUndefined(accountsBasedPermissions[data[i][1][j]]));
                        ...
                        if (!kony.sdk.isNullOrUndefined(accountsBasedPermissions[data[i][1][j]["accountID"]]["actions"])) {
                            ...
                            if (permissions.includes("TRANSFER_BETWEEN_OWN_ACCOUNT_CREATE") || permissions.includes("INTRA_BANK_FUND_TRANSFER_CREATE") || permissions.includes("INTER_BANK_ACCOUNT_FUND_TRANSFER_CREATE") || permissions.includes("INTERNATIONAL_ACCOUNT_FUND_TRANSFER_CREATE"));
                            else {
                                data[i][1].splice(j, 1);
...
define("MoneyMovementModule/frmMMP2PActivateSelectAccountsControllerActions", {
    /*
...
define("MoneyMovementModule/frmMMP2PActivateSelectAccountsController", ["MoneyMovementModule/userfrmMMP2PActivateSelectAccountsController", "MoneyMovementModule/frmMMP2PActivateSelectAccountsControllerActions"], function() {
    var controller = require("MoneyMovementModule/userfrmMMP2PActivateSelectAccountsController");
    var controllerActions = ["MoneyMovementModule/frmMMP2PActivateSelectAccountsControllerActions"];
