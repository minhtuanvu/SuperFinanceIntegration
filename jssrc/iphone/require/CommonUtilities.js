...
define(['ErrHandler', 'OLBConstants'], function(ErrHandler, OLBConstants) {
    ...
    var _getAccountDisplayName = function(account) {
        if (account.accountID === undefined || account.Account_id === undefined) return _getAccountName(account) + " ...." + _getLastFourDigit(account.PayPersonId);
    ...
    var _getFullName = function(Firstname, Middlename, Lastname) {
        if (Middlename !== undefined && Middlename !== "" && Middlename !== null) return Firstname + " " + Middlename + " " + Lastname;
    ...
    var _validateCurrency = function(amount) {
        if (amount) {
            ...
            if (amount.match(regularexp)) return true;
        }
        return false;
    };
    /**
    ...
    var _validateSearchString = function(searchString) {
        if (kony.sdk.isNullOrUndefined(searchString)) return "";
    ...
    var _getBackendDateFormat = function(dateString, dateFormat) {
        if (!dateFormat || !dateString) return;
        var date = dateString.split('/');
        if (dateFormat === 'dd/mm/yyyy') return date[2] + '-' + date[1] + '-' + date[0];
        else if (dateFormat === 'mm/dd/yyyy') return date[2] + '-' + date[0] + '-' + date[1];
    ...
    var _disableSegmentButtonSkinForCSRMode = function(size) {
        if (size == 13) return "sknBtnSSP3343A813PxBg0CSR";
        if (size == 15) return "sknBtnSSP3343A815PxBg0CSR";
        if (size == 17) return "sknBtnSSP3343A817PxBg0CSR";
    };
    /**
    ...
    var _isPrintEnabled = function() {
        return (applicationManager.getConfigurationManager().printingTransactionDetails === "true" && isDesktopWeb());
    }
    ...
    var _setText = function(widgetID, text, accessibilityConfig) {
            switch (typeof text) {
                case 'string':
                    ...
                    break;
                case 'object':
                    if (typeof text.text !== 'undefined') { // if text parameter is an object and contains some text
