define([], function() {
    ...
    Engage_PresentationController.prototype.checkForPendingMessageNotification = function() {
        if (this.notificationPending) {
            if (this.getUnreadMessageCount() > 0) {
                ...
                kony.timer.schedule("engageUnreadMessagesNotification", delayedMessageNotification.bind(this), 1, false);
            } else {
                this.notificationPending = false;
            }
        }
    ...
    Engage_PresentationController.prototype.dismissNewMessageNotification = function() {
        ...
        if (currentForm && currentForm.flxNotification) {
            ...
            currentForm.flxNotification.animate(kony.ui.createAnimation({
                "0": {
    ...
    Engage_PresentationController.prototype.displayNewMessageNotification = function() {
        ...
        if (currentForm) {
            if (applicationManager.getDeviceUtilManager().isIPhone()) {
                ...
                menuPresent = customFooter && imgMore && findWidgetVisibility(imgMore) === true;
            } else {
                var customHeader = findChildWidget(currentForm, "customHeader");
                var imgBack = findChildWidget(customHeader, "imgBack");
                menuPresent = customHeader && imgBack && findWidgetVisibility(imgBack) === true && imgBack.src.toLowerCase().indexOf("hamburger") >= 0;
            }
        }
        ...
        var msgTextResource = (messageCount > 1) ? "kony.mb.Engage.newMessages" : "kony.mb.Engage.newMessage";
        ...
        flxNotification.animate(kony.ui.createAnimation({
            "0": {
                "transform": transformObject0,
                "opacity": 0
            },
            ...
            if (provisioned === true) {
                ...
                var provisioningSuccess = function(response) {
                    ...
                    if (response.success == "true") {
                        applicationManager.getLoggerManager().log("Engage provisioning was successful, engageUserId = " + response.engageUserId);
                        this.launchEngage();
                    } else {
                        applicationManager.getLoggerManager().log("Engage provisioning was not successful, response = " + JSON.stringify(response));
                        ...
                        applicationManager.getDataProcessorUtility().showToastMessageError(controller, "Engage user provisioning was not successful, please try again later.");
                    }
                };
                var provisioningFailure = function(error) {
                    applicationManager.getPresentationUtility().dismissLoadingScreen();
                    applicationManager.getLoggerManager().log("Engage provisioning failed, error = " + JSON.stringify(error));
                    var currentForm = applicationManager.getNavigationManager().currentForm;
                    var controller = applicationManager.getPresentationUtility().getController(currentForm, true);
                    applicationManager.getDataProcessorUtility().showToastMessageError(controller, "Engage user provisioning failed, please try again later.");
                };
                applicationManager.getPresentationUtility().showLoadingScreen();
                applicationManager.getEngageManager().provisionEngage(provisioningSuccess.bind(this), provisioningFailure.bind(this));
            }
        }
    };
    /**
        ...
        for (var i = 0; i < menuData.length; i++) {
            ...
            if (menuItemText === applicationManager.getConfigurationManager().constants.MENUENGAGE) {
                if (engageUnreadCount > 0) {
                    menuData[i].text = {
                        "text": menuItemText,
                        "skin": "sknLbl0a78d1SSPSemiBold26px"
                    };
                    menuData[i].backGround = {
                        "isVisible": true
                    };
                    menuData[i].info = {
                        "text": engageUnreadCount.toString(),
                        "isVisible": true
                    };
                } else {
