define("CheckDepositModule/userfrmCheckDepositToController", ["CommonUtilities"], function(CommonUtilities) {
    return {
        segData: [],
        searchText: "",
        dataToAfterSearch: [],
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    for (var j = 0; j < data[i][1].length; j++) {
                        if (typeof(data[i][1][j][field]) == "string" && typeof(data[i][1][j].lblAccountName) == "string" && typeof(data[i][1][j].lblAccountNumber) == "string") {
                            if (kony.sdk.isNullOrUndefined(data[i][1][j][field]) !== undefined && data[i][1][j][field].toLowerCase().indexOf(searchText.toLowerCase()) >= 0 || data[i][1][j].lblAccountName.toLowerCase().indexOf(searchText.toLowerCase()) >= 0 || data[i][1][j].lblAccountNumber.toLowerCase().indexOf(searchText.toLowerCase()) >= 0) {
                                segEachData.push(data[i][1][j]);
                            }
                        }
                    }
