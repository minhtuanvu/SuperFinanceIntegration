define("com/wealth/common/investmentLineChart/userinvestmentLineChartController", function() {
    ...
    function setFilter(filter) {
        ...
        if (this.onFilterChanged) this.onFilterChanged(filter);
        else drawChart.call(this, filter);
    }
    ...
    function drawChart(chartType) {
        ...
        if (chartType === "1M") {
            ...
            maxVal = 5000;
        } else if (chartType === "5Y") {} else if (chartType === "YTD") {}
        try {
    ...
    function extractRefinitivData(filter, data) {
        ...
        switch (filter) {
            case '1D':
                break;
            case '1M':
                chartData = chartData.filter(d => checkWithinMonth(new Date(d.TIMESTAMP)));
                dateOptions = {
                    month: 'short',
                    day: 'numeric'
                };
                break;
            case '1Y':
                chartData = chartData.filter(d => checkWithinYear((new Date(d.TIMESTAMP)), 1));
                dateOptions = {
                    year: 'numeric',
                    month: 'short'
                };
                break;
            case '5Y':
                chartData = chartData.filter(d => checkWithinYear((new Date(d.TIMESTAMP)), 5));
                dateOptions = {
                    year: 'numeric',
                    month: 'short'
                };
                break;
            case 'YTD':
                chartData = chartData.filter(d => checkWithinYear((new Date(d.TIMESTAMP)), 0));
                dateOptions = {
                    year: 'numeric',
                    month: 'short'
                };
                break;
            default:
                chartData = chartData.filter(d => checkWithinYear(new Date(d.TIMESTAMP)));
                dateOptions = {
                    year: 'numeric',
                    month: 'short'
                };
...
define("com/wealth/common/investmentLineChart/investmentLineChartControllerActions", {
    /*
...
define("com/wealth/common/investmentLineChart/investmentLineChartController", ["com/wealth/common/investmentLineChart/userinvestmentLineChartController", "com/wealth/common/investmentLineChart/investmentLineChartControllerActions"], function() {
    var controller = require("com/wealth/common/investmentLineChart/userinvestmentLineChartController");
    var actions = require("com/wealth/common/investmentLineChart/investmentLineChartControllerActions");
    for (var key in actions) {
        controller[key] = actions[key];
    }
    controller.initializeProperties = function() {
        defineSetter(this, "skin1", function(val) {
            this.view.lblOneYear.skin = val;
        });
        defineGetter(this, "skin1", function() {
            return this.view.lblOneYear.skin;
        });
