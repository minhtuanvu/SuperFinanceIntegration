...
define(function() {
    ...
    function ValidationUtilManager() {
        this.phoneNumberRegex = /^[+]*[(]{0,1}[0-9]{1,3}[)]{0,1}[-\s\./0-9]*$/g;
    ...
    ValidationUtilManager.prototype.isValidCountryCode = function(CountryCode) {
        ...
        if (CountryCode === null || CountryCode === undefined || CountryCode === "" || CountryCode.length > 4) return false;
    ...
    ValidationUtilManager.prototype.isValidIBAN = function(iban) {
        ...
        for (ibanNum = '', i = 0; i < len; i += 1) ibanNum += parseInt(iban.charAt(i), 36);
        for (ibanMod = ibanNum.substr(0, 15) % 97, ibanNum = ibanNum.substr(15); ibanNum; ibanNum = ibanNum.substr(13)) ibanMod = (ibanMod + ibanNum.substr(0, 13)) % 97;
        return ibanMod == 1;
    };
