    ...
    return {
        ...
        setKeypadCharAccountNumber: function(char) {
            try {
                ...
                if (this.keypadStringAccountNumber.length > 0 && this.keypadStringAccountNumber.length < 13) {} else if (this.keypadStringAccountNumber.length < 1) {
                ...
                if (isValidAccNo) {
                    if (scope._beneficiaryTypes === "DomesticBank" || scope._beneficiaryTypes === "InternationalBank") {
                        ...
                        if (results.length === 0) {
                            ...
                            var unicode = "";
                            scope.UnifiedTransferDAO.invokeService(objSvcName, objName, operationName, criteria, unicode, scope.onAccountNumberSuccess, scope.failureValidation);
                        } else {
                            scope.view.lblReEnterAccountNumberErrorMsg.text = scope.getFieldValue(scope._accExistMsg);
                            scope.view.lblReEnterAccountNumberErrorMsg.setVisibility(true);
                            scope.incompleteCodeView();
        ...
        invokeSwiftServiceFromIBAN: function() {
            try {
                ...
                var criteria = scope.getCriteria();
                scope.UnifiedTransferDAO.invokeService(objSvcName, objName, operationName, criteria, "", scope.setSwiftCodeForNewPayee, scope.onSwiftServiceFailure);
            } catch (err) {
                var errObj = {
        ...
        invokeValidateIBANService: function() {
            try {
                ...
                var operationName = scope.getFieldValue(scope._accValidationOperation);
                var criteria = scope.getCriteria();
                scope.UnifiedTransferDAO.invokeService(objSvcName, objName, operationName, criteria, "", scope.onIBANSuccess, scope.failureValidation);
            } catch (err) {
                var errObj = {
        ...
        setPayeeNameDataTextChange: function() {
            try {
                ...
                if (scope.payeeNameTextChange == true) {
                    ...
                    if (Object.keys(minlength).length === 0 && minlength.constructor === Object && Object.keys(maxlength).length === 0 && maxlength.constructor === Object && Object.keys(payeeTextInput).length === 0 && payeeTextInput.constructor === Object) {
                        scope.enableButton("btnPayeeNameContinue");
                    } else {
                        scope.disableButton("btnPayeeNameContinue");
                    }
        ...
        onLinkPayeeContinue: function(selectedData, totalCustomers) {
            var scope = this,
                contractId, customerValue, totalCustomerSelectedCount = 0,
                dataLength = "",
                cif = [];
            try {
        ...
        setVerifyDetailsDefaultText: function() {
            try {
                var scope = this;
                if (scope._beneficiaryTypes == "SameBank") {
                    scope.inputPool["accountType"] = "Same Bank Account"
                }
                if (scope._beneficiaryTypes == "DomesticBank") {
                    scope.inputPool["accountType"] = "Domestic Account"
                }
                if (scope._beneficiaryTypes == "InternationalBank") {
                    scope.inputPool["accountType"] = "International Account"
                }
                if (scope._beneficiaryTypes == "PayAPerson") {
                    scope.inputPool["accountType"] = "Pay a Person"
                }
        ...
        setPhoneNumberDefaultText: function() {
            try {
                ...
                } else scope.view.flxContactsLookup.setVisibility(false);
                scope.disableButton("btnPhonerNumberContinue");
            ...
            try {
                ...
                scope.view.segCountryAlphabet.onRowClick = function() {
                    ...
                    if (selectedCodeNo !== null) {
                        ...
                        scope.view.flxCountryList.setContentOffset({
                            x: 0,
                            y: contentOffsetY
                        });
                    }
        ...
        setEmailAddressDefaultText: function() {
            try {
                ...
                } else scope.view.flxEmailLookup.setVisibility(false);
                if (scope.isEmptyNullUndefined(scope.getFieldValue(scope._cancelButton))) {
            ...
            try {
                ...
                scope.view.segCfcAlphabetsList.onRowClick = function() {
                    ...
                    if (selectedCodeNo !== null) {
                        ...
                        scope.view.flxCfcContactsList.setContentOffset({
                            x: 0,
                            y: contentOffsetY
                        });
                    }
        ...
        setChooseFromContactsSegmentData: function(data, flow) {
            try {
                ...
                if (!scope.isEmptyNullUndefined(data) && data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        if (!scope.isEmptyNullUndefined(data[i][
                                [flow]
                            ]) && data[i][
                                [flow]
                            ].length > 0) {
                            data[i].contactName = data[i].displayname[0].toUpperCase() + data[i].displayname.slice(1).toLowerCase() + "  ( " + data[i][
                                [flow]
                            ][0][
                                [id]
                            ] + " )";
...
define("com/temenos/infinityComponent/AddNewAccount/AddNewAccountControllerActions", {
    /*
        This is an auto generated file and any modifications to it may result in corruption of the action sequence.
    */
    AS_FlexContainer_g177081f5f6246ada9b37a27566eb517: function AS_FlexContainer_g177081f5f6246ada9b37a27566eb517(eventobject) {
        var self = this;
        return self.preShow.call(this);
    }
});
define("com/temenos/infinityComponent/AddNewAccount/AddNewAccountController", ["com/temenos/infinityComponent/AddNewAccount/userAddNewAccountController", "com/temenos/infinityComponent/AddNewAccount/AddNewAccountControllerActions"], function() {
    var controller = require("com/temenos/infinityComponent/AddNewAccount/userAddNewAccountController");
    var actions = require("com/temenos/infinityComponent/AddNewAccount/AddNewAccountControllerActions");
