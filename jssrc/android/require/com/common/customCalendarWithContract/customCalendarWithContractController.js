define("com/common/customCalendarWithContract/usercustomCalendarWithContractController", function() {
    return {
        ...
        preShow: function() {
            ...
            this.resetCal();
            this.holidays = "";
            if (!this.holidays) this.holidays = new Set();
            this.addHolidays();
            ...
            for (var i = 0; i < 6; i++) {
                ...
                for (var j = 0; j < 7; j++) {
                    ...
                    if (current.getTime() == startDate.getTime()) {
                        if (this.transitDays != 0) week[j].setEnabled(false);
                        week[j].skin = this.startDateRangeSkin;
                    } else if (current > startDate && current < deliverDate) {
                        if (week[j].skin == this.unSelectedDateSkin) week[j].skin = this.unSelectedRangeSkin;
                        else if (week[j].skin == this.diabledDateSkin) week[j].skin = this.disabledRangeSkin;
                        week[j].setEnabled(false);
                    } else if ((current.getTime() == deliverDate.getTime()) || (this.selectedDate !== '' && current > deliverDate && current < this.selectedDate)) {
                        if (week[j].skin == this.unSelectedDateSkin) week[j].skin = this.unSelectedRangeSkin;
                        else if (week[j].skin == this.diabledDateSkin) week[j].skin = this.disabledRangeSkin;
                    } else if (current < startDate) week[j].setEnabled(false);
                    else continue;
                }
            }
        },
        ...
        setDeliverDateToLabel: function() {
            ...
            if (currForm.id == "frmBillPayStartDate" && this.selectedDate) {
                ...
                if (transObj.frequencyType == "Once") currForm.lblDeliverDate.text = this.getDeliverDate(this.selectedDate);
                else {
                ...
                // this.getDeliverDate(this.selectedDate);
                currForm.lblDeliverDate.text = kony.i18n.getLocalizedString("kony.mb.BillPay.DeliveredIn") + this.transitDays + kony.i18n.getLocalizedString("kony.mb.BillPay.TransactionDays");
            } else if (currForm.id == "frmBillPayStartDate") currForm.lblDeliverDate.text = "";
        },
        getDeliverDate: function(selectedDate) {
            ...
            for (var i = 1; i <= this.transitDays; i++) {
                ...
                if (this.holidays.has(currDate)) holidaysInBetween++;
            }
            var counter = 1;
            ...
            this.view.flxNextMonth.onClick = function() {
                ...
                scope.view.flxMonth.widgets()[2].animate(kony.ui.createAnimation({
                    ...
                    "animationEnd": function() {
                        ...
                        scope.addHolidays();
                        if (kony.application.getCurrentForm().id == "frmBillPayEndDate") scope.setRangeSkins();
...
define("com/common/customCalendarWithContract/customCalendarWithContractControllerActions", {
    /* 
    This is an auto generated file and any modifications to it may result in corruption of the action sequence.
    */
});
define("com/common/customCalendarWithContract/customCalendarWithContractController", ["com/common/customCalendarWithContract/usercustomCalendarWithContractController", "com/common/customCalendarWithContract/customCalendarWithContractControllerActions"], function() {
    var controller = require("com/common/customCalendarWithContract/usercustomCalendarWithContractController");
    var actions = require("com/common/customCalendarWithContract/customCalendarWithContractControllerActions");
