define("com/temenos/infinityComponent/MakeATransfer/userMakeATransferController", ['./ParserUtilsManager', './UnifiedTransferDAO', './ValidationUtilManager', 'FormatUtil', './DataProcessorUtility'], function(ParserUtilsManager, UnifiedTransferDAO, ValidationUtilManager, FormatUtil, DataProcessorUtility) {
    return {
                        ...
                        if (fromAccountsDataKeys[i] == "Savings" || fromAccountsDataKeys[i] == "Checking" || fromAccountsDataKeys[i] == "Deposit" || fromAccountsDataKeys[i] == "Loan" || fromAccountsDataKeys[i] == "Credit Card") {
                            fromData = [{
                                ...
                                "flxUnifiedTransferHeader": {
                                    "skin": this._sknGroupHeaderBg
                                },
                                "flxUpShadow": {
                                    "skin": this._sknSegmentShadow
                                }
        ...
        groupResponseData: function(data, key) {
            var self = this;
            try {
                if (data !== undefined && data != "" && data != null) return data.reduce(function(value, obj) {
            ...
            try {
                ...
                for (var i = 0; i < keysItr.length; i++) {
                    ...
                    if (value instanceof Array) {
                        ...
                        if (isNaN(value) && (value.indexOf("{") > -1 || value.indexOf("[") > -1)) value = eval('(' + value + ')');
                        if (value instanceof Array) {
                            this.readArray(value, jsonPath);
                        } else if (value instanceof Object) {
                            this.readObject(value, jsonPath);
        ...
        rowExpandCollapse: function(context) {
            ...
            try {
                ...
                if (currentScreen == "flxToAccount") {
                    ...
                    if (this.segmentData === '') this.segmentData = JSON.parse(JSON.stringify(this.view.segFromAccounts.data));
        ...
        toAccRowExpandCollapse: function(context) {
            ...
            try {
                var sectionIndex = context.section;
                if (this.toAccSegmentData === '') this.toAccSegmentData = JSON.parse(JSON.stringify(this.view.segToAccounts.data));
        ...
        setTextBoxPlaceHolder: function(contractJSON, tbxWidget) {
            try {
                ...
                if (!(scope.isEmptyNullUndefined(contractJSON.placeHolder))) {
                    ...
                    scope.view[tbxWidget].placeholder = placeHolderValue ? placeHolderValue : "";
                }
            } catch (err) {
                var errObj = {
        ...
        onAccountNumberorIBANTextChange: function() {
            try {
                ...
                if (scope.flxNameAccountNoScreen === "flxInputAccNo") {
                    ...
                    var minlength = length["min"],
                        maxlength = length["max"],
                        title = scope.view.txtAccountNumberorIBAN.text;
                ...
                if (scope.flxNameAccountNoScreen === "flxReInputAccNo") {
                    ...
                    var minlength = length["min"],
                        maxlength = length["max"],
                        title = scope.view.txtReAccountNumberorIBAN.text;
                    if (title.length >= minlength && title.length <= maxlength) {
        ...
        enterCodePostAction: function() {
            try {
                if (this.flxNameAccountNoScreen === "flxInputAccNo") {
                    ...
                    var minlength = length["min"],
                        maxlength = length["max"],
                        title = this.keypadStringAccountNumber;
                    if (title.length >= minlength && title.length <= maxlength) {
        ...
        invokeSwiftServiceFromIBAN: function() {
            try {
                ...
                scope.UnifiedTransferDAO.getSwiftCode(objSvcName, objName, operationName, criteria, scope.setSwiftCodeForNewPayee, scope.onSwiftServiceFailure);
            } catch (err) {
                var errObj = {
        ...
        invokeValidateIBANService: function() {
            try {
                ...
                var criteria = scope.getCriteria();
                scope.UnifiedTransferDAO.validateIBAN(objSvcName, objName, operationName, criteria, scope.onIBANSuccess, scope.failureValidation);
            } catch (err) {
                var errObj = {
                ...
                if (isValidAccNo) {
                    if (scope.transferTypeContext === scope.getFieldValue(scope._transferTypes, "T2") || scope.transferTypeContext === scope.getFieldValue(scope._transferTypes, "T3")) {
                        ...
                        if (results.length === 0) {
                            ...
                            var criteria = scope.getCriteria();
                            scope.UnifiedTransferDAO.getBeneficiaryName(objSvcName, objName, operationName, criteria, scope.onAccountNumberSuccess, scope.failureValidation);
                        } else {
                            scope.view.lblReEnterAccNoErrorMsg.text = scope.getFieldValue(scope._accountNumberExistMsg);
                            scope.view.lblReEnterAccNoErrorMsg.setVisibility(true);
                            scope.incompleteCodeView();
        ...
        toAccSearch: function() {
            ...
            try {
                ...
                if (searchTxt != "") {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        for (var j = 0; j < 3; j++) {
                            if (!kony.sdk.isNullOrUndefined(data[i]["lblField" + (j + 1)]["text"]) && data[i]["lblField" + (j + 1)]["text"].toLowerCase().indexOf(searchTxt) >= 0) {
                                result.push(data[i]);
                                break;
                            }
                        }
        ...
        getBankIcon: function(bankName) {
            ...
            if (!kony.sdk.isNullOrUndefined(bankName)) {
                if (bankName.toLowerCase().includes("citi")) img = this.Icons.CITI_BANK_IMAGE;
                else if (bankName.toLowerCase().includes("chase")) img = this.IconsCHASE_BANK_IMAGE;
                else if (bankName.toLowerCase().includes("boa") || bankName.toLowerCase().includes("america")) img = this.Icons.BOA_BANK_IMAGE;
                else if (bankName.toLowerCase().includes("hdfc")) img = this.Icons.HDFC_BANK_IMAGE;
                else if (bankName.toLowerCase().includes("infintiy")) img = this.Icons.INFINITY_BANK_IMAGE;
                else img = this.Icons.EXTERNAL_BANK_IMAGE;
            }
            return img;
        },
        /*
                        ...
                        if (!this.isEmptyNullUndefined(FieldMapping.mapping[0])) {
                            ...
                            if (!fieldMapping.includes(",")) {
                                ...
                                for (var i = 0; i < fieldMapList.length; i++) {
                                    if (record[fieldMapList[i]]) data = record[fieldMapList[i]];
                                }
                            }
                        }
                    }
        ...
        onRecipientNameTextChange: function() {
            try {
                ...
                var minlength = length["min"],
                    maxlength = length["max"],
                    title = scope.view.txtRecipientName.text;
        ...
        contactCallBack: function(object) {
            try {
                ...
                if (parent_scope.contactTypeForContacts === "phone") {
                    if (resultContact["phone"]) scope.view.lblMobileNumber.text = resultContact["phone"];
                    scope.onContactTypeEmailDone();
                } else {
                    if (resultContact["email"]) scope.view.txtContactTypeEmail.text = resultContact["email"];
                    scope.onContactTypeEmailDone();
                }
            } catch (err) {
                var errObj = {
        ...
        onSuccessFetchCountriesList: function(response, unicode) {
            try {
                ...
                for (var each in response) {
                    var segmentData = {
                        "imgCountryCodeFlag": scope.getFieldValue(scope._countryFlagVisibility) ? {
                            "src": "french.png",
                            "isVisible": true
                        } : {
                            "isVisible": false
        ...
        getCurrencyForThirdPartyToAccount: function() {
            ...
            var operationName = scope.getFieldValue(scope._getBeneficiaryOperation);
            var criteria = scope.getCriteria();
            scope.UnifiedTransferDAO.getBeneficiaryName(objSvcName, objName, operationName, criteria, scope.onBeneficiaryDetailsSuccess, scope.failureValidation);
        },
        ...
        updateDateBullets: function(selectedDate) {
            var scope = this;
            try {
                if (!scope.isEmptyNullUndefined(selectedDate)) this.highlightedDate = this.FormatUtils.getFormatedDateString(selectedDate, "m/d/Y");
        ...
        setDate: function(flxname) {
            var scope = this,
                todaysDate = this.currentBankDate;
        ...
        validateCreditCardTransaction: function() {
            ...
            try {
                ...
                var criteria = Object.assign(scope.getCriteria(), {
                    "validate": true
                });
...
define("com/temenos/infinityComponent/MakeATransfer/MakeATransferControllerActions", {
    /*
        This is an auto generated file and any modifications to it may result in corruption of the action sequence.
    */
    AS_FlexContainer_a157b376de054fc0a10e9f6a63b2d74e: function AS_FlexContainer_a157b376de054fc0a10e9f6a63b2d74e(eventobject) {
        var self = this;
        this.preShow.call();
    },
    AS_FlexContainer_e32f8172086249c2a9837f5a68ded3a0: function AS_FlexContainer_e32f8172086249c2a9837f5a68ded3a0(eventobject) {
        var self = this;
        this.view.flxPayeeNickname.setVisibility(false);
        this.view.flxIntermediaryBIC.setVisibility(true);
    },
    AS_FlexContainer_f851b052deea419281ae7a6c2a4d937d: function AS_FlexContainer_f851b052deea419281ae7a6c2a4d937d(eventobject) {
        var self = this;
        this.view.flxPayeeNicknameSuccess.setVisibility(false);
        this.view.flxStartDate.setVisibility(true);
    }
});
define("com/temenos/infinityComponent/MakeATransfer/MakeATransferController", ["com/temenos/infinityComponent/MakeATransfer/userMakeATransferController", "com/temenos/infinityComponent/MakeATransfer/MakeATransferControllerActions"], function() {
    var controller = require("com/temenos/infinityComponent/MakeATransfer/userMakeATransferController");
    var actions = require("com/temenos/infinityComponent/MakeATransfer/MakeATransferControllerActions");
