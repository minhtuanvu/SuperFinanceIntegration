define([], function() {
    ...
    PresentationController.prototype.getNotificationsPresentationError = function(error) {
        ...
        if (err["isServerUnreachable"]) applicationManager.getPresentationInterruptHandler().showErrorMessage("postLogin", error);
    };
    ...
    PresentationController.prototype.searchAlerts = function(data, searchString) {
        ...
        if (!self.allAlertsChoosen) searchobj = self.searchWithCategories("notificationCategory", false, filters, ["notificationSubject", "notificationText"], searchString, data);
        else searchobj = self.searchWithCategories("notificationCategory", true, self.getDefaultFilterCategories(), ["notificationSubject", "notificationText"], searchString, data);
        var navManager = applicationManager.getNavigationManager();
    ...
    PresentationController.prototype.deleteNotificationSuccess = function(notificationId, response) {
        ...
        var newData = [];
        alertsData.forEach(function(data, index) {
            if (notificationId !== alertsData[index].userNotificationId) newData.push(data);
