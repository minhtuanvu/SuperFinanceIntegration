define("BillPayModule/userfrmBillPayFromAccountController", ["CommonUtilities"], function(CommonUtilities) {
    return {
        init: function() {
        ...
        setSegmentData: function() {
                ...
                if (isSingleCustomerProfile === false) {
                    if (profileAccess === "both") {
                        if (processedData[i].isBusinessAccount === "true") {
                            processedData[i].flxAccountType = {
                                "isVisible": true
                            };
                            processedData[i].imgAccountType = {
            ...
            for (var key in orgAccounts) {
                var headerJson = {
                    ...
                    "lblHeader": {
                        "text": key + " (" + orgAccounts[key].length + ")"
                    }
                };
            ...
            try {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    for (var j = 0; j < data[i][1].length; j++) {
                        if (typeof(data[i][1][j][field]) == "string" && typeof(data[i][1][j].processedAccountName) == "string" && typeof(data[i][1][j].bankName) == "string") {
                            if (data[i][1][j][field] !== undefined && data[i][1][j][field].toLowerCase().indexOf(searchText.toLowerCase()) >= 0 || data[i][1][j].processedAccountName.toLowerCase().indexOf(searchText.toLowerCase()) >= 0 || data[i][1][j].bankName.toLowerCase().indexOf(searchText.toLowerCase()) >= 0) {
        ...
        commonSegmentSearch: function(field, searchText, data) {
            var searchdata = [];
            for (var i = 0; i < data.length; i++) {
                if (typeof(data[i][field]) == "string" && typeof(data[i].processedAccountName) == "string" && typeof(data[i].bankName) == "string") {
                    if (data[i][field] !== undefined && data[i][field].toLowerCase().indexOf(searchText) >= 0 || data[i].processedAccountName.toLowerCase().indexOf(searchText) >= 0 || data[i].bankName.toLowerCase().indexOf(searchText) >= 0) {
            ...
            try {
                ...
                for (var key in processedAccounts) {
                    ...
                    for (var i = 0; i < processedAccounts[key].length; i++) {
                        ...
                        processedAccounts[key][i].flxAccountType = {
                            isVisible: iconVisible
                        };
                        processedAccounts[key][i].imgAccountType = {
                            src: imgIcon
                        };
                        processedAccounts[key][i].flxSeparator = {
                            isVisible: true
                        };
...
define("BillPayModule/frmBillPayFromAccountControllerActions", {
    /*
...
define("BillPayModule/frmBillPayFromAccountController", ["BillPayModule/userfrmBillPayFromAccountController", "BillPayModule/frmBillPayFromAccountControllerActions"], function() {
