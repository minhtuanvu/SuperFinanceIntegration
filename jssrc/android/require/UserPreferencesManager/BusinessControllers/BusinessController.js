...
define([], function() {
    ...
    userPreferencesManager.prototype.fetchUser = function(presentationSuccess, presentationError) {
        ...
        function  getAllCompletionCallback(status,  data,  error) {
            ...
            if (obj["status"] === true) {
                ...
                if (obj["data"][0]["EmailIds"] && obj["data"][0]["EmailIds"].length > 0) scope.userEntitlementsEmailIds = obj["data"][0]["EmailIds"];
                if (obj["data"][0]["ContactNumbers"] && obj["data"][0]["ContactNumbers"].length > 0) scope.userEntitlementsContactNumbers = obj["data"][0]["ContactNumbers"];
                if (obj["data"][0]["isSecurityQuestionConfigured"]) scope.isSecurityQuestionConfigured = true;
                if (obj["data"][0]["Addresses"]) scope.userEntitlementsAddresses = obj["data"][0]["Addresses"];
                if (obj["data"][0]["CoreCustomers"]) {
                    scope.isSingleCustomerProfile = obj["data"][0]["CoreCustomers"].length > 1 ? false : true;
                    obj["data"][0]["CoreCustomers"].forEach(function(item) {
                        ...
                        if (item.isPrimary === "true") scope.primaryCustomerId = {
                            id: item.coreCustomerID,
                            type: item.isBusiness === "true" ? 'business' : 'personal',
                            name: item.coreCustomerName || "",
                            contractId: item.contractId || "",
                    ...
                    scope.accessibleCustomerIds.forEach(function(item) {
                        if (!profiles.includes(item.type)) profiles.push(item.type);
                    });
                    if (profiles.length > 1) scope.profileAccess = "both";
                    else scope.profileAccess = profiles[0];
                }
    ...
    userPreferencesManager.prototype.fetchUserImage = function(presentationSuccess, presentationError) {
        var  userProfile  =  kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DbxUser");
        var scope = this;
    ...
    userPreferencesManager.prototype.isDeviceRegistered = function() {
        var sm = applicationManager.getStorageManager();
        var value = sm.getStoredItem("settingsObject");
        if (value) return value.deviceRegisterFlag == true ? true : false;
        else return false;
    };
    /**
    ...
    userPreferencesManager.prototype.isFirstTimeLoginUname = function() {
        var sm = applicationManager.getStorageManager();
        if (sm.getStoredItem("firstTimeLoginUname") === null || sm.getStoredItem("firstTimeLoginUname") === "") return true;
        else return false;
    };
    /**
    ...
    userPreferencesManager.prototype.resetSecurityQuestions = function(record, presentationSuccess, presentationError) {
        ...
        userObj.customVerb('resetSecurityQuestions', record, completionCallback);
    ...
    userPreferencesManager.prototype.updateUserPassword = function(record, presentationSuccess, presentationError) {
        var scope = this;
        var userAccount = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DbxUser");
        userAccount.customVerb('updateDBXUserPassword', record, completionCallback);
    ...
    userPreferencesManager.prototype.fetchEntitlementsForUser = function(params, presentationSuccess, presentationError) {
        ...
        function completionCallback(status, data, error) {
            ...
            var obj = srh.manageResponse(status, data, error);
            if (obj["status"] === true) {
                if (obj["data"]["EmailIds"].length > 0) scopeObj.userEntitlementsEmailIds = obj["data"]["EmailIds"];
                if (obj["data"]["ContactNumbers"].length > 0) scopeObj.userEntitlementsContactNumbers = obj["data"]["ContactNumbers"];
                if (obj["data"]["isSecurityQuestionConfigured"]) scopeObj.isSecurityQuestionConfigured = true;
                if (obj["data"]["Addresses"]) scopeObj.userEntitlementsAddresses = obj["data"]["Addresses"];
    ...
    userPreferencesManager.prototype.resetPassword = function(params, successCallBack, failureCallBack) {
        var userModel = kony.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition("DbxUser");
        userModel.customVerb("updateDbxCustomer", {
            "password": params
        }, completionCallBack);
    ...
    userPreferencesManager.prototype.updateUserStatus = function(inputParams, presentationSuccessCallback, presentationErrorCallback) {
        var userObj = kony.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository("DbxUser");
        userObj.customVerb('updateDBXUserStatus', inputParams, completionCallBack);
