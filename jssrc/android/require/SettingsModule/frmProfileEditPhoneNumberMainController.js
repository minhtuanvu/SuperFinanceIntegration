define("SettingsModule/userfrmProfileEditPhoneNumberMainController", ["CommonUtilities", "SCAUtility"], function(CommonUtilities, SCAUtility) {
    const SDKConstants = {
        "PIN_REQUEST": 100,
    ...
    return {
                        ...
                        if (!kony.sdk.isNullOrUndefined(segLayout)) {
                            for (var i = 0; i < segLayout.data.length; i++) {
                                if (segLayout.data[i].lblTitle.text === type && segLayout.data[i].imgPhoneNoTypeToggle.src === "radiobuttoninactive_big.png") {
                                    var checkedRadio = {
                                        lblTitle: {
                                            "text": segLayout.data[i].lblTitle.text
                                        },
                                        imgPhoneNoTypeToggle: {
                                            "src": "radiobtn_big.png"
                                        }
        ...
        validatePhone: function() {
            ...
            if (validationMan.isValidPhoneNumber(this.keypadString)) scopeObj.enterPostAction();
