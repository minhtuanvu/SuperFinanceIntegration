...
define([], function() {
    ...
    */
    ...
    CacheUtils.prototype._getPaginatedData = function() {
        var data = [];
        var scope = this;
        if (this._sortedData.length === 0) data = this.data.filter(this.filterPredicate).slice(parseInt(scope.offset), parseInt(scope.offset) + scope.pageSize).concat();
        else data = this._sortedData.filter(this.filterPredicate).slice(parseInt(scope.offset), parseInt(scope.offset) + scope.pageSize).concat();
        return data;
    };
    // helper method.
    ...
    CacheUtils.prototype._getFilteredData = function() {
        var scope = this;
        var data = this.data.filter(this.filterPredicate).slice((parseInt(scope.offset)), parseInt(scope.offset) + scope.pageSize).concat();
        data = this._sortWrapper(data);
        return data;
    };
    // helper method.
