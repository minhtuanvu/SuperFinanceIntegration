...
define([], function() {
    ...
    RegistrationManager.prototype.registerForPushNotifications = function() {
        if (applicationManager.getConfigurationManager().GoogleservicesSupported) this.registerToGCMorAPNS();
    };
    /**
    ...
    RegistrationManager.prototype.subscribeToMFMessagingService = function(regId, ostype) {
        ...
        try {
            ...
            applicationManager.getLoggerManager().log("subscribing to KMS, deviceId = " + deviceId + ", regId = " + regId + ", userId = " + userId);
    ...
    RegistrationManager.prototype.unSubscribeMFMessagingService = function() {
        try {
            var msgClient = kony.sdk.getCurrentInstance().getMessagingService();
            msgClient.unregister(function(response) {}, function(error) {});
        } catch (err) {}
    };
    /**
