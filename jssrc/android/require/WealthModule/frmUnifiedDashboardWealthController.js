define("WealthModule/userfrmUnifiedDashboardWealthController", ['CampaignUtility', 'CommonUtilities'], function(CampaignUtility, CommonUtilities) {
    return {
        /**
         * Description
         * @method preshow
        ...
        preshow: function() {
            ...
            var data = {};
            data = navigationMan.getCustomInfo('frmUnifiedDashboardWealth');
            this.wealthData = data.response;
            ...
            this.view.noAccounts.setVisibility(false);
        ...
        setAccountsInfo: function(internalAccounts, externalAccounts, bankName) {
            ...
            if (!kony.sdk.isNullOrUndefined(internalAccounts) && internalAccounts.length > 0) {
                internalAccounts.forEach(function(record) {
                    ...
                    if (record.isBusinessAccount === "false") {
        ...
        postShow: function() {
            ...
            //  this.loadAssetsDetails(this.wealthData);
            this.view.forceLayout();
            applicationManager.getPresentationUtility().dismissLoadingScreen();
        ...
        getCountsSuccessCB: function(singleApprovalsPendingCount, bulkApprovalsPendingCount, otherApprovalsPendingCount, singleRequestsPendingCount, bulkRequestsPendingCount, otherRequestsPendingCount, featureActionNames) {
            var configManager = applicationManager.getConfigurationManager();
            var checkUserPermission = function(permission) {
                return applicationManager.getConfigurationManager().checkUserPermission(permission);
            }
            ...
            if (isApprovalPermissionAccessible && isRequestPermissionAccessible) {
                if (Object.keys(featureActionNames).length !== 0) {
                    for (key in featureActionNames) {
                        for (var i = 0; i < featureActionNames[key].length; i++) {
                            if (featureActionNames[key][i].myApprovalsPending !== "0") {
                                json = {
                ...
                if (Object.keys(featureActionNames).length !== 0) {
                    for (key in featureActionNames) {
                        for (var i = 0; i < featureActionNames[key].length; i++) {
                            if (featureActionNames[key][i].myRequestsPending !== "0") {
                                json = {
                                    "lblSpendingCategory": featureActionNames[key][i].featureActionName,
            ...
            if (featureNameFinalArr.length === 0) {
                this.view.segRecentTransactions.isVisible = false;
                this.view.flxViewTransactions.isVisible = false;
                this.view.lblNoData.isVisible = true;
            } else {
                ...
                this.view.segRecentTransactions.setData(featureNameFinalArr);
            }
        },
        ...
        setupChartsDependingOnTypeOfUser: function() {
            ...
            try {
                ...
                if (this.wealthFlow === this.flowConstants.investmentBanking) {
                    this.setUpWealthAssetsChartDependingOnTypeOfUser(true);
                }
        ...
        loadInvestmentAccounts: function(respData) {
            ...
            var dataFromResponse = respData.PortfolioList.portfolioList;
        ...
        onInvestmentAccountSelect: function(event, context) {
            ...
            var allportfolioPermissionAccessible = configManager.getPortfolioPermissionsList();
            var isPortfolioPermissionAccessible = allportfolioPermissionAccessible.some(checkUserPermission);
            if (isPortfolioPermissionAccessible) {
                var navManager = applicationManager.getNavigationManager();
...
define("WealthModule/frmUnifiedDashboardWealthControllerActions", {
    /*
...
define("WealthModule/frmUnifiedDashboardWealthController", ["WealthModule/userfrmUnifiedDashboardWealthController", "WealthModule/frmUnifiedDashboardWealthControllerActions"], function() {
    var controller = require("WealthModule/userfrmUnifiedDashboardWealthController");
    var controllerActions = ["WealthModule/frmUnifiedDashboardWealthControllerActions"];
