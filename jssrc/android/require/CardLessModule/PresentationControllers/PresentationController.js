define(["AsyncManager/BusinessControllers/BusinessController"], function(AsyncManager) {
    ...
    CardLess_PresentationController.prototype.fetchAccountsSuccCallBack = function(res) {
        ...
        var data = res; //accountsManager.getCardLessWithdrawlSupportedAccounts();
    ...
    CardLess_PresentationController.prototype.pickContact = function() {
        ...
        var navManager = applicationManager.getNavigationManager();
        var cntType = scope_cardlessPresentationController.getCashlessContactType();
        if (cntType === "phone") scope_cardlessPresentationController.contactPickerObject.selectSinglePhoneNumber(scope_cardlessPresentationController.contactCallBack);
        else scope_cardlessPresentationController.contactPickerObject.selectSingleEmail(scope_cardlessPresentationController.contactCallBack);
    };
    CardLess_PresentationController.prototype.getCardlessPendingAndPostedTransactions = function() {
        ...
        if (navToForm !== "frmCardLessHome") {
            ...
            } else accountsManager.fetchInternalAccounts(scope_cardlessPresentationController.fetchAccountsSuccCallBackNew, scope_cardlessPresentationController.fetchAccountsErrCallBackNew);
        } else {
    ...
    CardLess_PresentationController.prototype.presentationDeleteACardlessTransferSuccess = function(res) {
        ...
        if (navigateTo === "frmAccountDetails") {
            ...
            if (applicationManager.getConfigurationManager().isAccountDetailsServiceConfigured) {
                var accountMod = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("AccountModule");
                accountMod.presentationController.fetchAccountDetailsAndTransactions(scope_cardlessPresentationController.getTransactionObject().fromAccountNumber);
            } else accountMod.presentationController.fetchAccountTransactions(scope_cardlessPresentationController.getTransactionObject().fromAccountNumber);
        } else {
    ...
    CardLess_PresentationController.prototype.getCardlessPendingAndPostedTransactionsQRScanner = function() {
        ...
        if (navToForm !== "frmCardLessHomeQR") {
            ...
            if (applicationManager.getConfigurationManager().isAccountDetailsServiceConfigured) {
                accountMod.presentationController.fetchAccountDetailsAndTransactions(scope_cardlessPresentationController.getTransactionObject().fromAccountNumber);
            } else accountMod.presentationController.fetchAccountTransactions(toAccountID);
        } else {
            applicationManager.getPresentationUtility().showLoadingScreen();
