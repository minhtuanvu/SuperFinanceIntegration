define("SavingsPotModule/userfrmCreateSavingsGoalFrequencyController", {
    preShow: function() {
    ...
    initActions: function() {
        ...
        var periodiccontri;
        if (applicationManager.getConfigurationManager().getDeploymentGeography() == "EUROPE") periodiccontri = parseFloat((String(result.periodicContribution)).split(",")[0].replace(".", ""));
        else periodiccontri = parseFloat((String(result.periodicContribution)).replace(",", ""));
    ...
    continueOnClick: function() {
        ...
        if (this.view.imgSelectMonthly.isVisible == true) {
            frequency = "Monthly";
            if (applicationManager.getConfigurationManager().getDeploymentGeography() == "EUROPE") periodicContribution = this.view.lblSelectedAmountValueMonthly.text.substring(1);
            else periodicContribution = formatUtil.deFormatAmount(this.view.lblSelectedAmountValueMonthly.text.substring(1));
            SavingsPotMod.presentationController.updatePeriodicContribution(periodicContribution);
        } else {
            frequency = "Biweekly";
            if (applicationManager.getConfigurationManager().getDeploymentGeography() == "EUROPE") periodicContribution = this.view.lblSelectedAmoutValueBiweekly.text.substring(1);
            else periodicContribution = formatUtil.deFormatAmount(this.view.lblSelectedAmoutValueBiweekly.text);
            SavingsPotMod.presentationController.updatePeriodicContribution(periodicContribution);
        }
        var flow = SavingsPotMod.presentationController.getSavingsFlow();
        if (flow) {
...
define("SavingsPotModule/frmCreateSavingsGoalFrequencyControllerActions", {
    /*
        This is an auto generated file and any modifications to it may result in corruption of the action sequence.
    */
    AS_Form_j457fe50e13f455aa2f85f344ac92afb: function AS_Form_j457fe50e13f455aa2f85f344ac92afb(eventobject) {
        var self = this;
        this.init();
    },
    AS_Form_c4e2ee9a30f44c21bd5292d763b1873d: function AS_Form_c4e2ee9a30f44c21bd5292d763b1873d(eventobject) {
        var self = this;
        this.preShow();
    },
    AS_BarButtonItem_j75a9599bf4a410b8f947e74b7c245df: function AS_BarButtonItem_j75a9599bf4a410b8f947e74b7c245df(eventobject) {
        var self = this;
        this.onCancelClick();
    }
});
define("SavingsPotModule/frmCreateSavingsGoalFrequencyController", ["SavingsPotModule/userfrmCreateSavingsGoalFrequencyController", "SavingsPotModule/frmCreateSavingsGoalFrequencyControllerActions"], function() {
    var controller = require("SavingsPotModule/userfrmCreateSavingsGoalFrequencyController");
    var controllerActions = ["SavingsPotModule/frmCreateSavingsGoalFrequencyControllerActions"];
