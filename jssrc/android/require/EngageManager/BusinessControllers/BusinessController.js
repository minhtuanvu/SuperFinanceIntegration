...
define([], function() {
    ...
    EngageManager.prototype.onSuccessfulLogin = function() {
        ...
        kony.application.addApplicationCallbacks({
            "onforeground": {
                "EngageUnreadItemRefresh": this.onForeground.bind(this)
            }
        });
    };
    /**
    ...
    EngageManager.prototype.fetchUnreadMessageCount = function() {
        ...
        var serviceSuccess = function(res) {
            if (res.success == "true" && !kony.sdk.isNullOrUndefined(res.unreadMessagesCount) && !isNaN(res.unreadMessagesCount)) {
                var unreadMsgCount = parseInt(res.unreadMessagesCount, 10);
                applicationManager.getLoggerManager().log("New unread message count fetched from Engage: " + unreadMsgCount);
                this.setUnreadMessageCount(unreadMsgCount, true);
            } else {
                applicationManager.getLoggerManager().log("Call to get Engage unread message count was not successful, response = " + JSON.stringify(res));
            }
        };
        var serviceFailure = function(err) {
            applicationManager.getLoggerManager().log("Call to get Engage unread message count returned error: " + JSON.stringify(err));
        };
        try {
            var service = kony.sdk.getCurrentInstance().getIntegrationService(serviceName);
            service.invokeOperation(operationName, headers, params, serviceSuccess.bind(this), serviceFailure.bind(this));
        } catch (error) {
    ...
    EngageManager.prototype.provisionEngage = function(successCallback, failureCallback) {
        ...
        try {
            var service = kony.sdk.getCurrentInstance().getIntegrationService(serviceName);
            service.invokeOperation(operationName, headers, params, provisionSuccess.bind(this), provisionFailure.bind(this));
        } catch (error) {
    ...
    EngageManager.prototype.receivedPushNotification = function(message, onlineOrOffline) {
        ...
        if (kony.sdk.isNullOrUndefined(newUnreadMessageCount) || isNaN(newUnreadMessageCount)) {
