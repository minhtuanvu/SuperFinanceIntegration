define("ExternalAccounts/userfrmExternalAccountsListController", {
    init: function() {
        ...
        if (applicationManager.getConfigurationManager().isAggregatedExternalAccountEnabled()) {
            ...
            if (externalAccounts.length > 0) {
                for (var i = 0; i < externalAccounts.length; i++) {
                    rowData = [];
                    rowData.push({
                        "displayAccountName": externalAccounts[i].accountName + "..." + (externalAccounts[i].accountID).substr((externalAccounts[i].accountID).length - 4),
                    ...
                    for (var j = i + 1; j < externalAccounts.length; j++) {
                        if (externalAccounts[j].bankName === externalAccounts[i].bankName) {
                            rowData.push({
                                "displayAccountName": externalAccounts[j].accountName + "..." + (externalAccounts[j].accountID).substr((externalAccounts[j].accountID).length - 4),
                    ...
                    allData.push([{
                        "BankName": externalAccounts[i].bankName,
                        "BankLogo": "genericexternalbank.png"
                    }, rowData]);
                }
            }
        }
    ...
    swipeDetected: function(widgetInfo, swipeInfo) {
        ...
        try {
            ...
            if (!kony.sdk.isNullOrUndefined(this.segTransactionsSwipeInfo)) this.segTransactionsSwipeInfo.isSwiped = false;
            loggerManager.log("#### in catch " + JSON.stringify(err) + " ####");
        }
    },
...
define("ExternalAccounts/frmExternalAccountsListControllerActions", {
    /*
        This is an auto generated file and any modifications to it may result in corruption of the action sequence.
    */
    AS_Form_hb365ca07a5f483aa1b10fbc1db05199: function AS_Form_hb365ca07a5f483aa1b10fbc1db05199(eventobject) {
        var self = this;
        this.init();
    },
    AS_Form_aa9f6885d2f143f7bb09aaa2f04e8140: function AS_Form_aa9f6885d2f143f7bb09aaa2f04e8140(eventobject) {
        var self = this;
        this.setInitialFormConfigurations();
    }
});
define("ExternalAccounts/frmExternalAccountsListController", ["ExternalAccounts/userfrmExternalAccountsListController", "ExternalAccounts/frmExternalAccountsListControllerActions"], function() {
    var controller = require("ExternalAccounts/userfrmExternalAccountsListController");
    var controllerActions = ["ExternalAccounts/frmExternalAccountsListControllerActions"];
