                                                ...
                                                if (!isNaN(parseFloat(limits[r]["value"]))) {
                                                    switch (oid) {
                                                        case "PRE_APPROVED_DAILY_LIMIT":
                                                            action["DAILY_APPROVE_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "AUTO_DENIED_DAILY_LIMIT":
                                                            action["DAILY_DENIAL_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "PRE_APPROVED_WEEKLY_LIMIT":
                                                            action["WEEKLY_APPROVE_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "AUTO_DENIED_WEEKLY_LIMIT":
                                                            action["WEEKLY_DENIAL_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "PRE_APPROVED_TRANSACTION_LIMIT":
                                                            action["PER_TRANSACTION_APPROVE_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "AUTO_DENIED_TRANSACTION_LIMIT":
                                                            action["PER_TRANSACTION_DENIAL_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "MAX_TRANSACTION_LIMIT":
                                                            action["MAX_TRANSACTION_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "DAILY_LIMIT":
                                                            action["DAILY_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                        case "WEEKLY_LIMIT":
                                                            action["WEEKLY_LIMIT"] = parseFloat(limits[r]["value"]);
                                                            break;
                                                    }
                                                } //limits end																				
                                            }
                                            if (kony.sdk.isNullOrUndefined(action["MAX_TRANSACTION_LIMIT"])) action["MAX_TRANSACTION_LIMIT"] = action["PER_TRANSACTION_DENIAL_LIMIT"];
                                            if (kony.sdk.isNullOrUndefined(action["DAILY_LIMIT"])) action["DAILY_LIMIT"] = action["DAILY_DENIAL_LIMIT"];
                                            if (kony.sdk.isNullOrUndefined(action["WEEKLY_LIMIT"])) action["WEEKLY_LIMIT"] = action["WEEKLY_DENIAL_LIMIT"];
                                            tempaction.push(action);
...
function deepCompareJSONObjects(a, b, f) {
    ...
    for (var d in b)
        if ("undefined" == typeof a[d]) return false;
    return true;
}
