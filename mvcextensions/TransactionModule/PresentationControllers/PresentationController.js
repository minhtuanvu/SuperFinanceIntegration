define(["AsyncManager/BusinessControllers/BusinessController"], function(AsyncManager) {
  ...
  Transaction_PresentationController.prototype.getBlockedTransactionsPresentationErrorCallback = function(resErr){
   ...
   Transaction_PresentationController.prototype.navigateToAccountDetails =function(res){
     if(res !== undefined && res.length >=0 ){
      ...
      if(currentForm ==="frmAdvanceSearch")
  ...
  Transaction_PresentationController.prototype.navToTransferLanding = function()
  {
    ...
    if(scope_Trans_Pre.formName)
      var data = navMan.getCustomInfo(scope_Trans_Pre.formName);
    else
    ...
    for(var i=0;i<transactions.scheduledTransactions.length;i++)
    {
      ...
      var accounttype;
      var isAccounttypeisVisible;
      var configurationManager = applicationManager.getConfigurationManager();
      if(configurationManager.isCombinedUser === "true") {
        isAccounttypeisVisible=true;
        if(transactions.scheduledTransactions[i]["isBusinessPayee"] === "0" ||transactions.scheduledTransactions[i]["isBusinessPayee"]===0){
          accounttype="personalaccount.png";
        }else{
          accounttype="businessaccount.png";
        }
        transactions.scheduledTransactions[i]["imgAccountType"] = accounttype;
      }else{
        isAccounttypeisVisible=false;
      }     
      transactions.scheduledTransactions[i]["flxAccountType"] = {isVisible:isAccounttypeisVisible};   
    ...
    for(var i=0;i<transactions.postedTransaction.length;i++)
    {
      ...
      if(configurationManager.isCombinedUser === "true") {
        isAccounttypeisVisible=true;
        if(transactions.postedTransaction[i]["isBusinessPayee"] === "0" ||transactions.postedTransaction[i]["isBusinessPayee"]===0){
          accounttype="personalaccount.png";
        }else{
          accounttype="businessaccount.png";
        }
        transactions.postedTransaction[i]["imgAccountType"] = accounttype;
      }else{
        isAccounttypeisVisible=false;
      }     
      transactions.postedTransaction[i]["flxAccountType"] = {isVisible:isAccounttypeisVisible}; 
