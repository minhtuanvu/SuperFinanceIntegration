define({
    ...
    preShow: function() {
        ...
        try {
            ...
            manageCardsModule.presentationController.getCardsList();
            var navManager = applicationManager.getNavigationManager();
            var currentForm = navManager.getCurrentForm();
    ...
    validatePin: function() {
  	...
  	/**
     * updateCardOperations - method for all update card operations
     */
  	updateCardOperations: function(cardData){
      this.cardList[this.cardListIndex] = cardData;
      if (cardData.view == "unlockCard") {
        this.updateCardAction = kony.i18n.getLocalizedString("i18n.CardManagement.UnlockCard");
        this.setFunctionalityForUnlockCard(cardData);
      } else if (cardData.view == "lockCard") {
        this.updateCardAction = kony.i18n.getLocalizedString("i18n.CardManagement.LockCard");
        this.setFunctionalityForLockCard(cardData);
      } else if (cardData.view == "pinChange") {
          if(cardData.cardType == "Credit"){
            this.updateCardAction = kony.i18n.getLocalizedString("i18n.CardManagement.ChangePinCredit");
          }
          else if(cardData.cardType == "Debit"){
            this.updateCardAction = kony.i18n.getLocalizedString("i18n.CardManagement.ChangePinDebit");
          }
          this.setFunctionalityForPinChange(cardData);
      } else if (cardData.view == "replaceCard") {
        this.updateCardAction = kony.i18n.getLocalizedString("i18n.CardManagement.ReplaceCard");
        this.setFunctionalityForReplaceCard(cardData);
      } else if (cardData.view == "lostCard") {
        this.updateCardAction = kony.i18n.getLocalizedString("i18n.CardManagement.ReportCard");
        this.setFunctionalityForLostCard(cardData);
      } else if (cardData.view == "cancelCard") {
        this.updateCardAction = kony.i18n.getLocalizedString("i18n.cardsManagement.CancelCard");
    ...
    lockUnlockCard: function(cardData) {
        try {
            applicationManager.getPresentationUtility().showLoadingScreen();
            var manageCardsModule = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("ManageCardsModule");
            manageCardsModule.presentationController.updateCardBasedOnAction(cardData,this.updateCardAction);
        } catch (err) {
            applicationManager.getPresentationUtility().dismissLoadingScreen();
    ...
    flxChangePinOnClick: function() {
        ...
        try {
            ...
            this.updateCardOperations(cardDetails);
        } catch (err) {
            applicationManager.getPresentationUtility().dismissLoadingScreen();
            throw GlobalExceptionHandler.addMessageAndActionForException(err, "kony.error.LoadingFormFailed", GlobalExceptionHandler.ActionConstants.ALERT, arguments.callee.name);
        }
    ...
    onReasonContinue: function() {
        ...
        if (cardData.view == "replaceCard" || cardData.view == "lostCard" || cardData.view == "cancelCard") {
            if (cardData.view == "replaceCard") {
              	reason = this.view.segReasonsReplaceCard.selectedRowItems[0].lblReason;
                this.setCardDetailsConfirmHeaderName("kony.tab.cardManage.replacingCard");
            }
            if (cardData.view == "lostCard") {
              reason = this.view.segReasonsStolenCard.selectedRowItems[0].lblReason;
                this.setCardDetailsConfirmHeaderName("kony.mb.cardManage.stolenCreditCard");
            }
            if (cardData.view == "cancelCard") {
              reason = this.view.segReasonsCancelCard.selectedRowItems[0].lblReason;
    ...
    flxOnClick: function() {
        ...
        try {
            ...
            if (kony.sdk.isNullOrUndefined(segData)) {
                kony.ui.Alert("Please select Reason");
                return;
            }
            segData = segData[0];
