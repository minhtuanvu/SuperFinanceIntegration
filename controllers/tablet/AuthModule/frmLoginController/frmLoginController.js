define({
  ...
  init: function(){
	...
	this.showErrorOrSuccessToastMessage();
	...
	showErrorOrSuccessToastMessage: function () {
    var navManager = applicationManager.getNavigationManager();
    var loginData = navManager.getCustomInfo("frmLoginToast");
    if (loginData && loginData.toastMessage && loginData.toastMessage !== "") {
      if(loginData.typeSuccess && loginData.typeSuccess !== ""){
        this.bindGenericSuccess(loginData.toastMessage);
        loginData.typeSuccess = "";
      }
      else{
  ...
  doLoginonTouchId : function ()
  {
    ...
    authMode.presentationController.currentAuthMode = "";
    authMode.presentationController.onLogin({"username":userName,"password":password}, this);
  },
