define({
  ...
  setMasterDataToSegment: function() {
    ...
    var segData = assets.map(function(item){
      return {
    ...
    if (self.selectedAssets !== -1) {
      segData[self.selectedAssets].flxMain.skin = "sknFlxF4F4F4Radius100";
    }
  ...
  processAndNavigate: function() {
    var self = this;
    var data = {
      "assets" : self.view.segDependents.selectedItems[0].selectedValue
    };
    var segData = self.view.segDependents.data;
    segData.forEach(function(item, index) {
      var type = item.selectedValue;
      item.flxMain.skin = type === data.assets ? "sknFlxF4F4F4Radius100" : "slFbox";
    });
    self.view.segDependents.setData(segData);
    var NUOMod = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("NewUserModule");
  ...
  assignDataToForm: function(newUserJSON){
    var assets = newUserJSON.assets ? newUserJSON.assets : "";
    var segData = this.view.segDependents.data;
    segData.forEach(function(item, index) {
      var type = item.selectedValue;
      item.flxMain.skin = type === assets ? "sknFlxF4F4F4Radius100" : "slFbox";
    });
